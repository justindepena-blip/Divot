import { NY_NJ_BBOX, OVERPASS_URL } from '../config'
import { distance, metresToYards } from './geo'
import type { Course, HoleData, LatLng, Position } from '../types'
...
/** `center` for ways/relations, the element itself for nodes. */
const pointOf = (el: OverpassElement): LatLng | null => {
 ...
 * Results are sorted by distance from the player when a fix is available.
 */
export async function searchCourses(
  ...
  found.sort((a, b) =>
    pos ? distance(pos, a) - distance(pos, b) : a.name.localeCompare(b.name),
  )
  return found
}
...
export interface CourseLayout {
  ...
  holeCount: number
  /** How many of the loaded holes came back with a green attached. */
  withGreens: number
}

/**
 * Holes, pars, lengths and greens for one course. Each hole is paired with its
 * nearest mapped green inside 700 yards — OSM rarely links the two explicitly.
 */
export async function loadCourseLayout(course: Course): Promise<CourseLayout> {
  ...
  return {
    ...
    withGreens: Object.values(holes).filter((h) => h.green).length,
  }
}
